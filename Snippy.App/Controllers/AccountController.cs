    ...
    public class AccountController : Controller
    {
            ...
            ForgotPasswordViewModel model)
        {
            if (this.ModelState.IsValid)
            {
                ...
                if (user == null
                    || !(await this.UserManager.IsEmailConfirmedAsync(user.Id)))
                {
            ...
            ResetPasswordViewModel model)
        {
            ...
            var result =
                await
                this.UserManager.ResetPasswordAsync(
                    user.Id,
                    model.Code,
                    model.Password);
            if (result.Succeeded)
            ...
            bool rememberMe)
        {
            ...
            var userFactors =
                await this.UserManager.GetValidTwoFactorProvidersAsync(userId);
            var factorOptions =
                userFactors.Select(
                    purpose =>
        ...
        public async Task<ActionResult> SendCode(SendCodeViewModel model)
        {
            ...
            if (
                !await
                 this.SignInManager.SendTwoFactorCodeAsync(
        ...
        {
            var loginInfo =
                await this.AuthenticationManager.GetExternalLoginInfoAsync();
            if (loginInfo == null)
            {
            ...
            var result =
                await this.SignInManager.ExternalSignInAsync(loginInfo, false);
            switch (result)
            {
                case SignInStatus.Success:
                    return this.RedirectToLocal(returnUrl);
            ...
            string returnUrl)
        {
            ...
            if (this.ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    result =
                        await
                        this.UserManager.AddLoginAsync(user.Id, info.Login);
                    if (result.Succeeded)
                    {
                        await this.SignInManager.SignInAsync(user, false, false);
                        return this.RedirectToLocal(returnUrl);
